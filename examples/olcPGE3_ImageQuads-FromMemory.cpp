...
public:
	...
	{
		std::ifstream imgTestStream("./assets/sanity_texture.png", std::ios::binary | std::ios::ate);
		std::vector<uint8_t> buffer;

		if(!imgTestStream.is_open())
			throw std::runtime_error{"failed to load sanity_texture.png"};

		buffer.resize(imgTestStream.tellg(), 0);
		imgTestStream.seekg(0, std::ios::beg);
		imgTestStream.read((char*)buffer.data(), buffer.size());
		imgTestStream.close();

		CreateImageFromMemory(imgTest, buffer.data(), buffer.size());
