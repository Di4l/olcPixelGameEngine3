    ...
    void Host_Linux_Wayland::pointer_frame(wl_pointer* pointer)
    {
        ...
        if (pointer_state.event_mask & wayland::PointerEventMask::PointerEventButton) {
            switch (pointer_state.button) {
                ...
                case BTN_BACK: pge_window->olc_OnMouseButton(3, pointer_state.state == WL_POINTER_BUTTON_STATE_PRESSED); break;
                case BTN_FORWARD: pge_window->olc_OnMouseButton(4, pointer_state.state == WL_POINTER_BUTTON_STATE_PRESSED); break;
                default: break;
            }
        }
